...
const isRevaluation = async () => {
  let res = await api.get(`/evaluation`)
  let response = res[res.length-1]

  console.log(res)

  if (response.is_reavaliation === 1) {
    return true
  } else return false
}
...
export default function Forms() {
  ...
  const [revalue, setRevalue] =  useState(isRevaluation);

  useEffect(() => {
    setRevalue(isRevaluation)
  }, []);
  ...
  const handleSubmit = (event) => {
    ...
    api.post(`forms`, {
      ...
      artigos_aceitos, artigos_preparacao, artigos_aguardando
    });
  }
